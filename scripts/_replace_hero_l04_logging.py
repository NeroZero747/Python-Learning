"""
Replace the hero section in lesson04_logging_basics.html with the correct template.
"""
  ...
  <div class="relative z-10 px-8 py-8 md:px-12 md:py-10">
    <div class="hero-split flex flex-col md:flex-row items-center gap-6 md:gap-10">
      ...
      <div class="flex-1 min-w-0">
        ...
        <!-- Title -->
        <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-3 leading-[1.15] tracking-tight">Logging Basics</h1>

        <!-- Subtitle -->
        <p class="text-white/80 text-sm md:text-base leading-relaxed mt-4 mb-5 max-w-prose">Learn how to use Python&#8217;s logging module to track errors, record events, and monitor your program &#8212; the professional alternative to print statements.</p>
        ...
        <div class="flex items-center gap-4 mb-5 text-sm">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-white/85 font-medium text-xs">March 30, 2026</span>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            ...
            <span class="font-extrabold">4<span class="font-bold opacity-50">/5</span></span>
...
start_idx = content.find(HERO_OPEN)
if start_idx == -1:
    print('ERROR: hero-container section not found!')
    exit(1)

# Find the matching closing </section> by depth counting
pos = start_idx + len(HERO_OPEN)
depth = 1
while depth > 0 and pos < len(content):
    next_open  = content.find('<section', pos)
    next_close = content.find('</section>', pos)
    if next_close == -1:
        print('ERROR: unmatched <section> tags!')
        exit(1)
...
print(f'Found hero: chars {start_idx}–{end_idx}')
print(f'Old hero length: {end_idx - start_idx} chars')

# Build the replacement — preserve surrounding whitespace prefix
# Find what indentation the section tag has
line_start = content.rfind('\n', 0, start_idx) + 1
indent = content[line_start:start_idx]
# Indent each line of new hero with the same whitespace
indented_hero = '\n'.join(
    (indent + line if line else line)
    for line in NEW_HERO.split('\n')
)

new_content = content[:start_idx] + indented_hero + content[end_idx:]

with open(TARGET, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f'New hero length: {len(indented_hero)} chars')
...
checks = [
    ('Lesson 04 label', 'Lesson 04' in result),
    ('Logging Basics title', 'Logging Basics' in result),
    ('fa6-solid:star icon', 'fa6-solid:star' in result),
    ('4/5 progress', '>4<span class="font-bold opacity-50">/5</span>' in result),
    ('Intermediate difficulty', 'Intermediate' in result),
    ('March 30 2026 date', 'March 30, 2026' in result),
    ('Calendar icon', 'fa6-regular:calendar' in result),
    ('4 Goals', '>4</span>\n            <span class="font-semibold opacity-55">Goals</span>' in result),
    ('4 Examples', '>4</span>\n            <span class="font-semibold opacity-55">Examples</span>' in result),
    ('3 Exercises', '>3</span>\n            <span class="font-semibold opacity-55">Exercises</span>' in result),
    ('SVG hex present', 'viewBox="0 0 280 324"' in result),
    ('hero-abstract-card opacity', 'padding:0.25rem;opacity:0.75;' in result),
    ('No fa6-solid:medal', 'fa6-solid:medal' not in result),
]
