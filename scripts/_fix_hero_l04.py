#!/usr/bin/env python3
"""Replace hero section in lesson04_lists_dictionaries.html."""
...
OPEN  = '<section class="hero-container">'
CLOSE = '</section>'

start = content.index(OPEN)
# Find the first </section> after the hero open (it's the hero close)
end   = content.index(CLOSE, start) + len(CLOSE)
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4">
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            <span class="iconify text-[10px]" data-icon="fa6-solid:cubes"></span> Module 2
          </span>
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
        ...
        <!-- Title -->
        <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-3 leading-[1.15] tracking-tight">Lists &amp; Dictionaries</h1>

        <!-- Subtitle -->
        <p class="text-white/80 text-sm md:text-base leading-relaxed mt-4 mb-5 max-w-prose">Discover how Python lists and dictionaries work — two essential tools for storing and organising data in every real Python program.</p>
        ...
        <div class="flex items-center gap-4 mb-5 text-sm">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-white/85 font-medium text-xs">March 25, 2026</span>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            ...
            <span class="font-extrabold">4<span class="font-bold opacity-50">/9</span></span>
...
new_content = content[:start] + NEW_HERO + content[end:]
...
checks = [
    ("hero-container section present", '<section class="hero-container">'),
    ("Module 2 badge", ">Module 2<"),
    ("cubes icon", "fa6-solid:cubes"),
    ("Beginner dots (green+gray+gray)", "#22c55e"),
    ("6 min read", "6 min read"),
    ("Lesson 04 label", ">Lesson 04<"),
    ("Title: Lists & Dictionaries", "Lists &amp; Dictionaries"),
    ("Subtitle starts with Discover", "Discover how Python lists"),
    ("Author line", "Python Learning Hub"),
    ("Date separator |", 'text-white/30">|</span>'),
    ("Calendar icon", "fa6-regular:calendar"),
    ("Date: March 25, 2026", "March 25, 2026"),
    ("Goals=4", ">4</span>\n            <span class=\"font-semibold opacity-55\">Goals</span>"),
    ("Examples=4 stat", "fa6-solid:code"),
    ("Exercises=3", ">3</span>"),
    ("Progress 4/9", ">4<span"),
    ("hex SVG present", 'viewBox="0 0 280 324"'),
    ("hero-abstract-card style", 'style="padding:0.25rem;opacity:0.75;"'),
    ("No old subtitle text", "Programming Foundations &middot; Python Foundations Track"),
]
print("=== Verification ===")
for label, needle in checks:
    if label == "No old subtitle text":
        ok = needle not in final
        print(f"{'OK' if ok else 'FAIL'} — old generic subtitle removed")
    else:
        ok = needle in final
