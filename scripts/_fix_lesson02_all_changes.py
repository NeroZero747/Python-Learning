...
1. TOC module list (15 lessons, lesson02 active)
2. Key takeaways redesign (2-col grid, text-sm, no modifier classes)
...
if not old_toc_m:
    print('⚠️  TOC block not found — skipping (already applied)')
elif 'lesson02_reading_files_csv_excel_json.html" class="flex items-center gap-2 px-3 py-2 rounded-lg border bg-[#fdf0f7]' in old_toc_m.group() and 'lesson15_end_to_end_project.html' in old_toc_m.group():
    print('⚠️  TOC already correct — skipping')
else:
...
old_ki_m = re.search(r'<div class="bg-white px-8 py-7 space-y-4">.*?</div>\s*</div>\s*</section>', content, re.DOTALL)
# This could match multiple — make sure we get the key-ideas one
# Find by offset after key-ideas section open tag
...
if not ki_body_m:
    print('⚠️  key-ideas body not found — skipping (already applied)')
else:

# Replace within the full content
old_ki_str = ki_body_m.group()
# Verify it only appears once in the file
if content.count(old_ki_str) != 1:
    print(f'❌ key-ideas body appears {content.count(old_ki_str)} times — not safe to replace')
    sys.exit(1)

new_ki_full = NEW_KI_BODY + '\n  </div>\n</section>'
content = content.replace(old_ki_str, new_ki_full, 1)
...
# 3. REAL-WORLD SECTION REDESIGN — health insurance, file-reading pipeline
# ─────────────────────────────────────────────────────────
NEW_RW_BODY = '''<div class="bg-white px-8 py-7 space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="relative rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md hover:border-emerald-100 transition-all duration-300">
        ...
        <div class="px-4 py-4">
          ...
          <p class="text-xs font-bold text-gray-800 mb-1.5">Step 4 — Inspect all three DataFrames</p>
          <p class="text-xs text-gray-500 leading-relaxed mb-3">Runs <code class="bg-gray-100 px-0.5 rounded">df.head()</code> and <code class="bg-gray-100 px-0.5 rounded">df.info()</code> on each DataFrame to confirm row counts, column names, and data types before any analysis begins.</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="rounded-2xl border border-violet-100 bg-white overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
        ...
        <div class="px-5 py-5 space-y-3">
          ...
          <p class="text-xs text-gray-600 leading-relaxed">Receives a daily pipe-delimited CSV extract of all processed claims. Loads it with <code class="text-[11px] bg-gray-100 px-1 rounded font-mono">pd.read_csv(sep="|", encoding="utf-8")</code> to stack 12 regional files into a single DataFrame for trend analysis.</p>
...
# Find the section body div
rw_body_m = re.search(r'<div class="bg-white px-8 py-7 space-y-[0-9]+">.*?</div>\s*</div>\s*</section>', rw_region, re.DOTALL)
if not rw_body_m:
...
if content.count(old_rw_str) != 1:
    print(f'❌ real-world body appears {content.count(old_rw_str)} times — not safe to replace')
    sys.exit(1)

new_rw_full = NEW_RW_BODY + '\n    </div>\n  </div>\n</section>'
