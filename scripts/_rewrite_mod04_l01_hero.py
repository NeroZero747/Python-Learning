"""Rewrite <section class="hero-container"> in lesson01_creating_your_own_modules.html."""
import pathlib, sys

TARGET = pathlib.Path("pages/track_01_python_foundation/mod_04_python_best_practices/lesson01_creating_your_own_modules.html")
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4">
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            <span class="iconify text-[10px]" data-icon="fa6-solid:star"></span> Module 4
          </span>
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
          ...
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            <span class="iconify text-[10px]" data-icon="fa6-regular:clock"></span> 5 min read
          </span>
        </div>

        <!-- Lesson number label -->
        <p class="text-xs font-bold uppercase tracking-[0.2em] text-white/90 mb-2">Lesson 01</p>

        <!-- Title -->
        <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-3 leading-[1.15] tracking-tight">Creating Your Own Modules</h1>

        <!-- Subtitle -->
        <p class="text-white/80 text-sm md:text-base leading-relaxed mt-4 mb-5 max-w-prose">Learn how to split your Python scripts into separate, reusable module files that you can import into any project you build.</p>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            ...
            <span class="font-extrabold">1<span class="font-bold opacity-50">/5</span></span>
...
# Find the hero section by marker
START_MARKER = '<section class="hero-container">'
start = html.find(START_MARKER)
if start == -1:
    print("❌ hero-container not found"); sys.exit(1)

# Depth-count <section>...</section> to find the matching close
search = html[start:]
depth, end_rel, i = 0, -1, 0
while i < len(search):
...
old_hero = html[start:start + end_rel]
print(f"  Old hero: {len(old_hero):,} chars")
print(f"  New hero: {len(NEW_HERO):,} chars")

html = html[:start] + NEW_HERO + html[start + end_rel:]
TARGET.write_text(html, encoding="utf-8")
print(f"✅ Written: {len(html):,} chars total")

# ── Verify ─────────────────────────────────────────────────────────────
result = TARGET.read_text(encoding="utf-8")
hero_start = result.find('<section class="hero-container">')
hero_end = result.find("</section>", hero_start) + len("</section>")
s = result[hero_start:hero_end]

checks = [
    ("Module icon: fa6-solid:star",             'data-icon="fa6-solid:star"'),
    ("Module number: Module 4",                 "> Module 4"),
    ("Old medal icon gone",                     "fa6-solid:medal" not in s),
    ("Difficulty: Intermediate",                ">Intermediate<" in s or "Intermediate\n" in s or "Intermediate " in s),
    ("Intermediate: 2 green dots",              s.count("#22c55e") == 2),
    ("Intermediate: 1 gray dot",                s.count("#d1d5db") == 1),
    ("Read time: 5 min read",                   "5 min read"),
    ("Lesson label: Lesson 01",                 ">Lesson 01<"),
    ("Old Lesson 02 gone",                      "Lesson 02" not in s),
    ("Title: Creating Your Own Modules",        ">Creating Your Own Modules<"),
    ("Subtitle present (15-25 words)",          "Learn how to split your Python scripts into separate"),
    ("Author: Python Learning Hub",             "Python Learning Hub"),
    ("Date separator |",                        'class="text-white/30">|<'),
    ("Date: March 29, 2026",                    "March 29, 2026"),
    ("fa6-regular:calendar",                    'data-icon="fa6-regular:calendar"'),
    ("Goals: 4",                                "<span class=\"font-extrabold\">4</span>\n            <span class=\"font-semibold opacity-55\">Goals</span>"),
    ("Examples: 4",                             "<span class=\"font-extrabold\">4</span>\n            <span class=\"font-semibold opacity-55\">Examples</span>"),
    ("Exercises: 3",                            "<span class=\"font-extrabold\">3</span>\n            <span class=\"font-semibold opacity-55\">Exercises</span>"),
    ("Progress: 1/5",                           ">1<"),
    ("Progress: /5",                            ">/5<"),
    ("Old progress 2/5 gone",                   ">2<" not in s),
    ("hero-abstract-card padding:0.25rem",      'style="padding:0.25rem;opacity:0.75;"'),
    ("SVG present",                             'viewBox="0 0 280 324"'),
    ("SVG PYTHON text",                         ">PYTHON<"),
    ("SVG LEARNING HUB text",                   ">LEARNING HUB<"),
    ("Background layers: hero-dots",            'class="hero-dots"'),
    ("Background layers: hero-glow-1",          'class="hero-glow hero-glow-1"'),
    ("Background layers: hero-glow-line",       'class="hero-glow-line"'),
    ("fa6-solid:dumbbell (exercises icon)",     'data-icon="fa6-solid:dumbbell"'),
    ("href anchors preserved",                  'href="#objective"' in s and 'href="#code-examples"' in s and 'href="#practice"' in s),
]
