"""Rewrite the hero section in lesson02_project_folder_structure.html.

Inputs:
  MODULE_NUM      = 4
  MODULE_ICON     = fa6-solid:star  (Module 4+)
  DIFFICULTY      = Beginner  (Track 1 default; 1 green dot)
  READ_TIME       = 5 min read
  LESSON_NUM_LABEL= Lesson 02
  LESSON_TITLE    = Project Folder Structure
  SUBTITLE        = Discover how splitting a Python project into organised folders
                    — with separate places for scripts, data, and settings —
                    makes your work easier to find and update.  (25 words)
  PUB_DATE        = March 29, 2026
  GOALS_COUNT     = 4
  EXAMPLES_COUNT  = 4  (4 ce-step tabs)
  EXERCISES_COUNT = 3  (3 pe-step tabs)
  LESSON_POSITION = 2
  TOTAL_LESSONS   = 5
"""
  ...
  <div class="relative z-10 px-8 py-8 md:px-12 md:py-10">
    <div class="hero-split flex flex-col md:flex-row items-center gap-6 md:gap-10">
      ...
      <div class="flex-1 min-w-0">
        ...
        <!-- Title -->
        <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-3 leading-[1.15] tracking-tight">Project Folder Structure</h1>

        <!-- Subtitle -->
        <p class="text-white/80 text-sm md:text-base leading-relaxed mt-4 mb-5 max-w-prose">Discover how splitting a Python project into organised folders — with separate places for scripts, data, and settings — makes your work easier to find and update.</p>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="hero-pill inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs">
            ...
            <span class="font-extrabold">2<span class="font-bold opacity-50">/5</span></span>
...
def main():
    ...
    start = html.index(hero_open)
    # Find the matching close — it's the first </section> after the hero_open
    end = html.index(hero_close, start) + len(hero_close)

    old_hero = html[start:end]
    html = html[:start] + NEW_HERO + html[end:]

    TARGET.write_text(html, encoding="utf-8")
    print(f"✅  hero replaced  ({len(old_hero):,} chars → {len(NEW_HERO):,} chars)")
