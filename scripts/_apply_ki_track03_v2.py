"""
Restore track_03 lesson files from git (old module numbering) to current
filesystem (new module numbering), then re-apply just the #key-ideas body
with the corrected regex that does NOT eat the rest of the file.
"""
import os
import re
import subprocess
...
# ─── Map: current filesystem folder → git (HEAD) folder ────────────
FOLDER_MAP = {
    "mod_01_data_engineering_foundations":          "mod_02_data_engineering_foundations",
    "mod_02_nosql_and_modern_data_storage":         "mod_03_nosql_and_modern_data_storage",
    "mod_03_api_data_integration":                  "mod_04_api_data_integration",
    "mod_04_data_pipelines_and_orchestration":       "mod_05_data_pipelines_and_orchestration",
    "mod_05_large_scale_data_processing":            "mod_06_large_scale_data_processing",
    "mod_06_automation_and_ci_cd":                   "mod_07_automation_and_ci_cd",
}

def git_show(git_path):
    """Return file content from HEAD via git show."""
...
# ─── Step 1: Restore every file from git ────────────────────────────
print("=== STEP 1: Restoring files from git ===")
restored = 0
not_in_git = 0

for current_folder, git_folder in sorted(FOLDER_MAP.items()):
    current_dir = os.path.join(BASE, current_folder)
    if not os.path.isdir(current_dir):
        print(f"  SKIP dir not found: {current_dir}")
        continue
    for fname in sorted(os.listdir(current_dir)):
        if not fname.endswith(".html"):
            continue
        git_path = f"{BASE}/{git_folder}/{fname}".replace("\\", "/")
        content = git_show(git_path)
        if content is None:
            print(f"  ⚠️  Not in git: {git_path}")
            not_in_git += 1
            continue
        fs_path = os.path.join(current_dir, fname)
        with open(fs_path, "w", encoding="utf-8") as fh:
            fh.write(content)
        restored += 1

print(f"  Restored {restored} files, {not_in_git} not found in git\n")


# ─── Step 2: Re-apply key-ideas with FIXED regex ───────────────────
print("=== STEP 2: Applying key-ideas cards ===")

# Full lesson data (same as before)
LESSONS = {
...
def replace_key_ideas_body(content, new_body):
    """
    FIXED regex: Replace ONLY the inner body div of #key-ideas.
    Strategy: find the <div class="bg-white px-8 py-7 space-y-4"> inside
    #key-ideas and replace everything up to its matching closing </div>.
    We count div nesting to find the correct closing tag.
    """
    # Find the key-ideas section
    section_start = content.find('<section id="key-ideas"')
    if section_start == -1:
        return None
    ...
    section_content = content[section_start:section_end + len('</section>')]

    # Find the body div within the section
    body_marker = '<div class="bg-white px-8 py-7 space-y-4">'
    body_start = section_content.find(body_marker)
    if body_start == -1:
        return None

    # Count nested divs to find the matching close
    pos = body_start + len(body_marker)
    depth = 1
    while depth > 0 and pos < len(section_content):
        next_open = section_content.find('<div', pos)
        next_close = section_content.find('</div>', pos)
        if next_close == -1:
            break
    ...
    # Replace just the body div
    old_body = section_content[body_start:body_end]
    new_section = section_content.replace(old_body, new_body, 1)

    return content[:section_start] + new_section + content[section_start + len(section_content):]
...
def main():
    ...
    for rel, data in sorted(LESSONS.items()):
        ...
        # Verify we didn't lose sections
        old_sections = re.findall(r'<section id="([^"]+)"', content)
        new_sections = re.findall(r'<section id="([^"]+)"', new_content)
        if old_sections != new_sections:
            print(f"  [FAIL] SECTION MISMATCH: {rel}")
            print(f"       Old: {old_sections}")
            print(f"       New: {new_sections}")
            fail += 1
            continue

        with open(fp, "w", encoding="utf-8") as fh:
            fh.write(new_content)
        print(f"  [OK] {rel}" + (" (+ CSS fix)" if did_fix_css else ""))
        ok += 1

    print(f"\nDone: {ok} patched, {fail} failed, {css_fixed} CSS fixes")
